...
icrt_export(DVts, Vt, {Tag,Attrs}) ->
    {_File,Loc} = loc(Attrs),
    Vts = [lists:sort(dict:to_list(DVt)) || DVt <- DVts],
    icrt_export(lists:merge(Vts), lists:sort(dict:to_list(Vt)),
		{Tag,Loc}, length(Vts), vtnew()).
...
icrt_export([{V,{{export,_},_,_}}|Vs0], [{V,{{export,_}=S0,_,Ls}}|Vt],
    ...
    icrt_export(Vs, Vt, In, I, dict:store(V, {S,used,Ls}, Acc));
icrt_export([{V,_}|Vs0], [{V,{_,_,Ls}}|Vt], In, I, Acc) ->
    ...
    icrt_export(Vs, Vt, In, I, dict:store(V, {bound,used,Ls}, Acc));
...
icrt_export([{V,_}|_]=Vs0, Vt, In, I, Acc) ->
    ...
    icrt_export(Vs, Vt, In, I, dict:store(V, {S,U,Ls}, Acc));
...
vtunsafe({Tag,FileLine}, Uvt, Vt) ->
    ...
    dict:fold(fun (V, {_, U, Ls}, Acc) ->
		dict:store(V, {{unsafe,{Tag,Line}},U,Ls}, Acc)
	      end, vtnew(), vtnew(Uvt, Vt)).
...
vtvar(V, Info) -> dict:store(V, Info, vtnew()).
...
vt_no_unused(Vt) ->
    dict:filter(fun (_, {_,U,_L}) -> U =/= unused end, Vt).
